...
function PRE_deleteServerComment(auth_email, req_body) {
  ...
  if (!safeEmail(auth_email)) return `PRE_deleteServerComment cook name does not match safeEmail() ${cook}`;
...
function POST_deleteServerComment(delete_comment_promise) {
  ...
  delete_comment_promise.then((delete_comment) => {
    is_resolved = true;
    const type_issue = type_czech.checkParam_type(delete_comment, SPEC_DELETE_COUNT);
    if (type_issue) return type_issue;
  });
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_deleteServerComment D did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
...
function PRE_deleteServerRecipe(auth_email, req_body) {
  ...
  if (!safeEmail(auth_email)) return `PRE_deleteServerRecipe cook name does not match safeEmail() ${auth_email}`;
...
function POST_deleteServerRecipe(delete_recipe_promise) {
  ...
  let is_resolved = false;
  delete_recipe_promise.then((delete_recipe) => {
    is_resolved = true;
    const type_issue = type_czech.checkParam_type(delete_recipe, SPEC_DELETE_COUNT);
    if (type_issue) return type_issue;
  });
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_deleteServerRecipe D did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
...
function PRE_addServerComment(auth_email, req_body) {
  ...
  if (type_issue) return type_issue;
  if (!safeEmail(auth_email)) return `PRE_addServerComment cook name does not match safeEmail() ${auth_email}`;
...
function POST_addServerComment(add_comment_promise) {
  ...
  let is_resolved = false;
  add_comment_promise.then((reTitle_recipe) => {
    is_resolved = true;
  ...
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_addServerComment D did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
...
function PRE_reTitleServerRecipe(auth_email, req_body) {
  ...
  if (type_issue) return type_issue;
  if (!safeEmail(auth_email)) return `PRE_reTitleServerRecipe cook name does not match safeEmail() ${auth_email}`;
...
function POST_reTitleServerRecipe(reTitle_recipe_promise) {
  ...
  let is_resolved = false;
  reTitle_recipe_promise.then((reTitle_recipe) => {
    is_resolved = true;
  ...
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_reTitleServerRecipe D did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
...
function PRE_changeServerRecipe(auth_email, req_body) {
  ...
  if (type_issue) return type_issue;
  if (!safeEmail(auth_email)) return `PRE_changeServerRecipe cook name does not match safeEmail() ${auth_email}`;
...
function POST_changeServerRecipe(change_recipe_promise) {
  ...
  let is_resolved = false;
  change_recipe_promise.then((added_recipe) => {
    is_resolved = true;
  ...
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_addServerRecipe D did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
...
function PRE_addServerRecipe(auth_email, req_body) {
  const type_issue = type_czech.checkParam_type([auth_email, req_body], ["string", SPEC_RECIPE_TYPES]);
  if (type_issue) return type_issue;
  if (!safeEmail(auth_email)) return `PRE_getCookComments cook name does not match safeEmail() ${auth_email}`;
...
function POST_addServerRecipe(add_recipe_promise) {
  ...
  let is_resolved = false;
  add_recipe_promise.then((added_recipe) => {
    is_resolved = true;
    const type_issue = type_czech.checkParam_type(added_recipe, SPEC_RECIPE_TYPES);
    if (type_issue) return type_issue;
    const empty_issue = type_czech.checkParam_emptyEither(added_recipe, SPEC_RECIPE_EMPTIES);
...
function POST_postToDb(change_promise) {
  ...
  let is_resolved = false;
  change_promise.then((recipe_data) => {
    is_resolved = true;
  ...
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_postToDb D did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
...
function PRE_getFromDb(get_filter_request) {
  ...
  if (num_slash !== 5) return `PRE_getFromDb did not match filter url '${get_url}'`;
  const get_filter_url = "/" + FILTER_FIRST_SECTION + "/";
  if (!get_url.startsWith(get_filter_url)) return `PRE_getFromDb did not start with /'${FILTER_FIRST_SECTION}'`;
}
...
function POST_getFromDb(recipes_promise) {
  ...
  recipes_promise.then((filtered_recipes) => {
    ...
    if (type_err) type_czech.check_assert(`POST_getFromDb B ` + type_err);
    const empty_issue = type_czech.checkArray_objEmpty0n(sorted_recipes, SPEC_RECIPE_EMPTIES);
    if (empty_issue) type_czech.check_assert(`POST_getFromDb C ` + empty_issue);

    const count_err = type_czech.checkParam_type(count_recipes, "number");
    if (count_err) type_czech.check_assert(`POST_getFromDb D ` + count_err);
  });
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_getFromDb D did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
