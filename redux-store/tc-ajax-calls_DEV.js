...
import { SPEC_POST_TO_DB_TYPE, SPEC_RECIPE_TYPES, SPEC_RECIPE_EMPTIES } from "../import-2-require/tc-types-2-import";
...
function PRE_databasePost(db_request, shared_csrfToken, json_data) {
  ...
  if (shared_csrfToken === "") type_czech.check_assert(`PRE_databasePost A an empty csrf value ${shared_csrfToken}`);
}
...
function POST_databasePost(get_cook_promise) {
  ...
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_databasePost D did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
...
function POST_databaseGet(recipes_promise) {
  ...
  recipes_promise.then((filtered_recipes) => {
    ...
    if (type_err) type_czech.check_assert(`POST_databaseGet B did not resolve to an array, but ` + type_err);

    const empty_issue = type_czech.checkArray_objEmpty0n(sorted_recipes, SPEC_RECIPE_EMPTIES);
    if (empty_issue) type_czech.check_assert(`POST_databaseGet C did not resolve to an array, but ` + empty_issue);
    const count_err = type_czech.checkParam_type(count_recipes, "number");
    if (count_err) type_czech.check_assert(`POST_databaseGet D ` + count_err);
  });
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_databaseGet E did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
