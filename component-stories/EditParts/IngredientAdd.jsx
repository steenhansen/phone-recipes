...
function IngredientAdd({ clickAddIngred }) {
  ...
  return (
    <div key={the_key} className="pb-4">
      <div className=" w-[25px] mr-1 float-left clear-both pb-2">&nbsp;</div>

      <div className="float-left w-6/12 mr-1">
        <input maxLength="26" className="w-full base-edit" onChange={addIngredientChange} placeholder="Lukewarm milk..." type="text" value={add_ingredient} />
      </div>

      <div className="float-left w-3/12">
        <input maxLength="12" className="w-full base-edit" onChange={addAmountChange} placeholder="2 cups..." type="text" value={add_amount} />
      </div>
