...
function RemarkRow(a_remark) {
  ...
  const [_recipe_id, _recipe_title, _cooks_email, your_comment] = the_parts;
  return (
    <div key={the_key} className="m-1">
      <ButtonBase className=" cut-red">
        <a onClick={clickCutRecipe}>Cut</a>
      </ButtonBase>
      <a className="ml-1 base-link" href={user_url}>
        {title}
      </a>{" "}
...
function CookRemarks({ cooks_remarks }) {
  ...
  return (
    <>
      ...
      <div className="ml-4">{the_remarks}</div>
    </>
  );
}
