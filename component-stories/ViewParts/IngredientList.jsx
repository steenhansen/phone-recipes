...
function ListRowSimple(ingredient_set, index, row_input_colour = "bg-row-input-colour-none") {
  ...
  return (
    <div key={the_key} className="mt-2 ">
      <div className="float-left w-3/4 pb-1 ">
        {" "}
        <input className={`base-view w-full ${row_input_colour} `} type="text" defaultValue={ingredient} disabled />
      </div>
      <div className="float-left w-1/4 pl-2 ">
        <input className={`base-view w-full ${row_input_colour} `} type="text" defaultValue={amount} disabled />
      </div>
    </div>
  );
}
