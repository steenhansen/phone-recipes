...
function SignInOrCreate({ setCreateRecipe }) {
  ...
  if (shared_auth_email === '') {
    ...
    href_link = <a id="--create--recipe--" onClick={_ => setCreateRecipe(true)}>Create Recipe</a>
  } else {
    const jump_to_create = user_url + '?q=create';
    href_link = <a id="--create--recipe--" href={jump_to_create}>Create Recipe</a>
  }
  const top_button2 = (
    <div className="-mt-4 base-width">
      <ButtonBase className="block mx-auto" >
        {href_link}
      </ButtonBase>
      ...
      <React.StrictMode>
        <CreateContextProvider create_variables={create_variables} >
          <FilterContextProvider filter_variables={filter_variables} >
            <div id='recipe-page-section' >  {pages_section} </div>
