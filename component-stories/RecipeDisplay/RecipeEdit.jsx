...
import { PRE_RecipeEdit } from "../../type-czech-jsx/RecipeDisplay/tc_RecipeEdit";
...
function getRecipe(edited_values) {
  const { server_variables, the_title, the_steps, the_serves, the_time, the_meal, the_cuisine, the_diet, the_ingredients, the_internal, the_minutes } =
    edited_values;
  const the_cook = server_variables.shared_auth_email;
  const trimmed_title = the_title.trim();
  const lower_title = trimmed_title.toLowerCase();
  const _id = the_cook + ID_SEPARATOR + lower_title + ID_SEPARATOR;
  const title_steps = " " + trimmed_title + " " + the_steps + " ";
  const the_search = the_ingredients.reduce((acc, curr) => acc + curr.ingredient + " " + curr.amount + " ", title_steps);
...
function RecipeEdit({ is_storybook, filtered_recipes, the_recipe, setEditRecipe }) {
  ...
  const initial_ingredients = ingredients.reduce((acc, curr) => acc + curr.ingredient + " " + curr.amount + " ", "");

  const current_ingredients = the_ingredients.reduce((acc, curr) => acc + curr.ingredient + " " + curr.amount + " ", "");
