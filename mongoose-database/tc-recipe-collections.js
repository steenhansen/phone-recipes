...
const {
  SPEC_INGREDIENT_TYPE, SPEC_INGREDIENT_EMPTY,
...
function POST_changeRecipe(recipe_promise) {
  ...
  recipe_promise.then(
    changed_recipe => {
      ...
      if (type_err)
        type_czech.check_assert(` is not a correct recipe, but ` + type_err);
      const empty_issue = type_czech.checkParam_emptyEither(changed_recipe, [SPEC_RECIPE_EMPTIES, SPEC_TITLE_EMPTY])
      if (empty_issue)
        type_czech.check_assert(`POST_changeRecipe C has EMPTY errors ` + empty_issue);
    });

  setTimeout(() => is_resolved ? 'POST_changeRecipe D just fine thank you'  // ''
...
function POST_reTitleRecipe(re_title_promise) {
  ...
  re_title_promise.then(
    re_titled => {
      is_resolved = true;

      const type_err = type_czech.checkParam_type(re_titled, SPEC_RECIPE_TYPES);
      if (type_err)
        type_czech.check_assert(`POST_reTitleRecipe B is not a correct recipe, but ` + type_err);

      const empty_issue = type_czech.checkParam_empty(re_titled, SPEC_RECIPE_EMPTIES)
      if (empty_issue)
        type_czech.check_assert(`POST_reTitleRecipe C has EMPTY errors ` + empty_issue);


    });

  setTimeout(() => is_resolved ? 'POST_reTitleRecipe D just fine thank you'  // ''
...
function PRE_safeSearch(safe_title, steps, safe_ingredients) {
  ...
  //    const type_issue_ing = type_czech.checkArray_objType0n(safe_ingredients, { ingredient: 'string', amount: 'string' });
  ...
  //  const empty_issue_ing = type_czech.checkArray_objEmpty0n(safe_ingredients, { ingredient: 'EMPTY-ERROR', amount: 'EMPTY-ERROR' });
  const empty_issue_ing = type_czech.checkArray_objEmpty0n(safe_ingredients, SPEC_INGREDIENT_EMPTY);
  if (empty_issue_ing)
    return empty_issue_ing
}
...
function PRE_newRecipe(new_recipe) {
  ...
  if (type_issue)
    return type_issue                     ///                   this is corret when delete a comment
...
function POST_newRecipe(save_promise) {
  ...
  save_promise.then(
    saved_recipe => {
      ...
      if (type_err)
        type_czech.check_assert(`POST_newRecipe B is not a correct recipe, but ` + type_err);

      const empty_issue = type_czech.checkParam_empty(saved_recipe, SPEC_RECIPE_EMPTIES)
      if (empty_issue)
        type_czech.check_assert(`POST_newRecipe C has EMPTY errors ` + empty_issue);

    });

  setTimeout(() => is_resolved ? 'POST_newRecipe D just fine thank you'
...
function PRE_getOneRecipe(recipe_id) {
  ...
  if (!safeEmail(cook))
    return `PRE_getOneRecipe A Cook name does not match safeEmail() -  - ${cook}`
  if (!title.length > 0)
    return `PRE_getOneRecipe B title is empty  - ${title}`
}
...
function POST_getOneRecipe(delete_promise) {
  ...
  delete_promise.then(
    recipe_array => {
      ...
      if (array_err)
        array_err.check_assert(`POST_getOneRecipe B is not a correct recipe, but ` + array_err);
      if (recipe_array.length !== 0) {
        const type_err = type_czech.checkParam_type(recipe_array[0], SPEC_RECIPE_TYPES);
        if (type_err)
          type_czech.check_assert(`POST_getOneRecipe C is not a correct recipe, but ` + type_err);
        const empty_issue = type_czech.checkParam_empty(recipe_array[0], SPEC_RECIPE_EMPTIES)
        if (empty_issue)
          type_czech.check_assert(`POST_getOneRecipe D has EMPTY errors ` + empty_issue);
      }
    });
  setTimeout(() => is_resolved ? 'POST_getOneRecipe D just fine thank you'  // ''
...
function PRE_deleteRecipe(recipe_id) {
  ...
  const [cook, title] = recipe_arr;
  if (!safeEmail(cook))
    return `PRE_deleteRecipe A Cook name does not match safeEmail() -  - ${cook}`
  if (!title.length > 0)
    return `PRE_deleteRecipe B title is empty  - ${title}`
}
...
function POST_deleteRecipe(delete_promise) {
  ...
  delete_promise.then(
    delete_counts => {
      ...
      if (type_issue)
        type_czech.check_assert(`POST_deleteRecipe B is not a correct recipe, but ` + type_issue);
    });

  setTimeout(() => is_resolved ? 'POST_deleteRecipe C just fine thank you'  // ''
...
function PRE_getCooksRecipes(cook) {
  ...
  if (type_issue)
    return type_issue
  if (!safeEmail(cook))
    return `Cook name does not match safeEmail() - recipes - ${cook}`
}
...
function POST_getCooksRecipes(cook_promise) {
  ...
  cook_promise.then(
    cook_recipes => {
      ...
      if (type_err)
        type_czech.check_assert(`POST_getCooksRecipes B ` + type_err);
      const empty_issue = type_czech.checkArray_objEmpty0n(sorted_recipes, SPEC_RECIPE_EMPTIES);
      if (empty_issue)
        type_czech.check_assert(`POST_getCooksRecipes C ` + empty_issue);

      const count_err = type_czech.checkParam_type(count_recipes, 'number');
      if (count_err)
        type_czech.check_assert(`POST_getCooksRecipes D ` + count_err);
  ...
  setTimeout(() => is_resolved ? ''
    : type_czech.check_assert(`POST_getCooksRecipes E did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)
    , MAX_TEST_AJAX_DELAY_SEC);
}
...
function POST_getFilteredRecipes(recipes_promise) {
  ...
  recipes_promise.then(
    filtered_recipes => {
      ...
      const type_err = type_czech.checkArray_objType0n(sorted_recipes, SPEC_RECIPE_TYPES);
      if (type_err)
        type_czech.check_assert(`POST_getFilteredRecipes B ` + type_err);
      const empty_issue = type_czech.checkArray_objEmpty0n(sorted_recipes, SPEC_RECIPE_EMPTIES);
      if (empty_issue)
        type_czech.check_assert(`POST_getFilteredRecipes C ` + empty_issue);

      const count_err = type_czech.checkParam_type(count_recipes, 'number');
      if (count_err)
        type_czech.check_assert(`POST_getFilteredRecipes D ` + count_err);


    });
  setTimeout(() => is_resolved ? ''
    : type_czech.check_assert(`POST_getFilteredRecipes E did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)
    , MAX_TEST_AJAX_DELAY_SEC);
}
...
function PRE_remakeOldComments(poss_comments, new_title) {
  const type_err = type_czech.checkArray_objType0n(poss_comments, SPEC_RECIPE_COMMENTS_TYPE);
  if (type_err)
    type_czech.check_assert(`PRE_remakeOldComments B ` + type_err);
  const empty_issue = type_czech.checkArray_objEmpty0n(poss_comments, SPEC_RECIPE_COMMENTS_EMPTY);
  if (empty_issue)
    type_czech.check_assert(`PRE_remakeOldComments C ` + empty_issue);

  const type_title = type_czech.checkParam_type(new_title, 'string');
  if (type_title)
    return type_title
  const empty_title = type_czech.checkParam_empty(new_title, 'EMPTY-ERROR')
  if (empty_title)
    return empty_title
}
...
function POST_remakeOldComments(save_promise) {
  ...
  save_promise.then(
    comment_ids => {
      ...
      if (type_err)
        type_czech.check_assert(`POST_remakeOldComments B is not a correct recipe, but ` + type_err);

    });

  setTimeout(() => is_resolved ? 'POST_remakeOldComments D just fine thank you'  // ''
