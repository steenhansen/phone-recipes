...
function POST_readConfig(the_result) {
  ...
  const result_empties = {
    ...
    HIDDEN_CREDENTIALS: {
      MONGO_URI: "EMPTY-ERROR",
      GOOGLE_CLIENT_ID: "EMPTY-ERROR",
      GOOGLE_CLIENT_SECRET: "EMPTY-ERROR",
      SESSION_SECRET: "EMPTY-ERROR",
    },
  };
...
function POST_getMongoCred(the_result) {
  ...
  if (type_issue) return type_issue;
  if (!the_result.startsWith("mongodb:")) return "POST_getMongoCred, error mongodb didn't return valid " + the_result;
}
...
function POST_dbConnect(the_result) {
  ...
  if (Recipes_coll.collection?.name !== "recipes") return "POST_dbConnect got wrong recipe collection name : " + Recipes_coll.collection.name;
  if (Comments_coll.collection?.name !== "comments") return "POST_dbConnect got wrong comments collection name : " + Comments_coll.collection.name;
}
