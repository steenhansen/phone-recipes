...
function PRE_getCookComments(cook) {
  ...
  if (!safeEmail(cook)) return `PRE_getCookComments cook name does not match safeEmail() ${cook}`;
}
...
function POST_getCookComments(get_cook_promise) {
  ...
  get_cook_promise.then((cook_comments) => {
    is_resolved = true;

    const type_err = type_czech.checkArray_objType0n(cook_comments, SPEC_RECIPE_COMMENTS_TYPE);
    if (type_err) type_czech.check_assert(`POST_getCookComments B ` + type_err);

    const empty_issue = type_czech.checkArray_objEmpty0n(cook_comments, SPEC_RECIPE_COMMENTS_EMPTY);
    if (empty_issue) type_czech.check_assert(`POST_getCookComments C ` + empty_issue);
  });
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_getCookComments D did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
...
function POST_removeComment(del_comment_promise) {
  ...
  let is_resolved = false;
  del_comment_promise.then((del_comment) => {
    is_resolved = true;
  ...
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_removeComment C did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
...
function POST_deleteCommentsOnRecipe(recipe_comments_promise) {
  ...
  let is_resolved = false;
  recipe_comments_promise.then((recipe_comments) => {
    is_resolved = true;
  ...
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_deleteCommentsOnRecipe C did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
...
function POST_addComment(add_comments_promise) {
  ...
  let is_resolved = false;
  add_comments_promise.then((recipe_comments) => {
    is_resolved = true;
  ...
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_addComment C did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
...
function POST_getOneComment(add_comments_promise) {
  ...
  let is_resolved = false;
  add_comments_promise.then((one_comment) => {
    is_resolved = true;
  ...
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_getOneComment C did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
...
function POST_getRecipeComments(recipe_comments_promise) {
  ...
  recipe_comments_promise.then((recipe_comments) => {
    ...
    if (type_err) type_czech.check_assert(`POST_getRecipeComments B ` + type_err);

    const empty_issue = type_czech.checkArray_objEmpty0n(recipe_comments, SPEC_RECIPE_COMMENTS_EMPTY);
    if (empty_issue) type_czech.check_assert(`POST_getRecipeComments C ` + empty_issue);
  });
  setTimeout(
    () => (is_resolved ? "" : type_czech.check_assert(`POST_getRecipeComments D did not return within ${MAX_TEST_AJAX_DELAY_SEC} milliseconds`)),
    MAX_TEST_AJAX_DELAY_SEC
  );
}
