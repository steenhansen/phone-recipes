...
function CommentRow(a_comment) {
  ...
  const remark_test_id = testIdStrip(remark);
  return (
    <div key={the_key}>
      "<span id={remark_test_id}>{remark}</span>"<span className="italic text-sm">- commented by </span>  <a className="base-link italic text-sm " href={cook_page}>{email}</a>
    </div>
  )
}
